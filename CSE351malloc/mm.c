...
void* mm_malloc (size_t size) {
  ...
  size_t* NxtBlkFtrPtr = NULL;  
  BlockInfo *NxtBlkPtr = NULL;
  ...
  if (size <= MIN_BLOCK_SIZE) {
	...
	// get a free block that is large enough based off required size 
   ptrFreeBlock = searchFreeList(reqSize); 
	
	if(ptrFreeBlock == NULL) {
		//if there is no free space of required size, allocate more space on heap
	...
	// if size of free block is smaller than
	if(blockSize < reqSize + MIN_BLOCK_SIZE){  
		reqSize = blockSize;  
	} 
	//sets the next used block tag
	precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;  
	
	//set  the sizeAndTags of the free block to be used
	ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED; 		  
	
	// set pointer of the next block
	NxtBlkPtr = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock,reqSize);  		    
	
	// if allocated space might be smaller than the free space, split
	if(reqSize != blockSize){  
		// set the head of the block this block to be free
		NxtBlkPtr->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;  
		
		// set the footer of the block to be free
		NxtBlkFtrPtr = (size_t *)UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize - WORD_SIZE);  							    
		(*NxtBlkFtrPtr) = NxtBlkPtr->sizeAndTags;  									  
		
		// put block on the free list (LIFO)
		insertFreeBlock(NxtBlkPtr);  
	} else {  
		// set header of next block to be the preceding tag. need to allocate the block
		NxtBlkPtr->sizeAndTags = NxtBlkPtr->sizeAndTags | TAG_PRECEDING_USED;  							  
		
		// set the footer
		NxtBlkFtrPtr = (size_t*)UNSCALED_POINTER_ADD(NxtBlkPtr,SIZE(NxtBlkPtr->sizeAndTags) - WORD_SIZE);  								   
		(*NxtBlkFtrPtr) = NxtBlkPtr->sizeAndTags;  
	}  
...
void mm_free (void *ptr) {
  ...
  // above.  They are included as minor hints.
	...
	//Change the block to unused (free)
	followingBlock->sizeAndTags = blockInfo->sizeAndTags ^ TAG_USED;	 				
	
	//Change the tag to be free
	blockInfo->sizeAndTags = blockInfo->sizeAndTags ^ TAG_USED;	 							
	
	//Adjust following block size and tags
	followingBlock = (BlockInfo*)(UNSCALED_POINTER_ADD(followingBlock, WORD_SIZE));		
